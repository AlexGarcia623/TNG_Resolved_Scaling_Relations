...
with h5py.File(data, 'r') as f:
    ...
    for key in all_subhalos.keys():
        ...
        this_gmsd  = np.array(ret(this_subhalo['GasMass']))
...
plt.hist2d(all_sm  , all_gm, bins=(75,75), cmap=cmr.get_sub_cmap('Greys', 0, 0.8), 
           rasterized=True, norm='log', alpha=0.5, zorder=2)

mass, sfr, sfr1, sfr2 = median(all_sm, all_gm)
...
for k in range(len(titles)):
    ...
    mass, sfr, sfr1, sfr2 = median(all_sm[within_dm], all_gm[within_dm])
...
plt.ylabel(r'$\log(\Sigma_{\rm gas}~[M_\odot/{\rm kpc^2}])$')
...
plt.tight_layout()
plt.savefig('./figs/Figure9.pdf', bbox_inches='tight')