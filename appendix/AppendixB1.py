...
list_of_centrals = np.load('../first_sub.npy')
list_of_centrals = [f'subhalo_{sub}' for sub in list_of_centrals]

all_keys = []
with h5py.File(data, 'r') as f:
    all_subhalos = f['snap_99']
    all_keys = list(all_subhalos.keys())

centrals   = np.intersect1d(all_keys,list_of_centrals,assume_unique=True)
satellites = np.setdiff1d(all_keys  ,list_of_centrals,assume_unique=True)

print(len(centrals),len(satellites),len(all_keys))
...
for index, which in enumerate([all_keys, centrals, satellites]):
    counter = 0
    ...
    if index == 0:
        ax.hist2d(all_sm  , all_sfr, bins=(100,100), cmap=cmr.get_sub_cmap('Greys', 0, 0.8), 
    ...
    if index == 0:
        print(f'All fit:', poly)
        label = r'${\rm Full~Sample}$'
        ls = '-'
    elif index == 1:
        print(f'Central fit:', poly)
        label = r'${\rm Centrals}$'
        ls = '--'
    else:
        print(f'Satellite fit:', poly)
...
plt.tight_layout()
plt.savefig('./figs/AppendixB1.pdf', bbox_inches='tight')