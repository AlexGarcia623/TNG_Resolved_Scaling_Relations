...
plt.hist2d(all_sm  , all_sfr, bins=(75,75), cmap=cmr.get_sub_cmap('Greys', 0, 0.8), 
           rasterized=True, norm='log', alpha=0.5, zorder=2)

mass, sfr, sfr1, sfr2 = median(all_sm, all_sfr)

ax.plot(mass, sfr, color='k', alpha=0.5, label=r'${\rm Median~rSFMS}$', lw=8)
ax.plot(mass, sfr1, color='k', alpha=0.5, lw=4)
ax.plot(mass, sfr2, color='k', alpha=0.5, lw=4)
ax.fill_between(mass, sfr1, sfr2, color='gray', alpha=0.25)
